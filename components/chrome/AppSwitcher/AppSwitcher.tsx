...
export interface LauncherApp { label: string; icon: string; /** Optional stable id passed back to onLaunch. */ key?: string; }
...
export interface AppSwitcherProps {
  ...
  /** Fired when an app tile or quick action is chosen. The launcher closes unless you preventDefault. */
 ...
 * onPinChange(apps) — fired when user pins/unpins. onLaunch(app) — a tile or
 * quick action was chosen.
 *
 * A modal dialog (labelled, focus trapped with the search box focused first,
 * Escape on the dialog, page scroll locked, focus returned on close). Each tile
 * is a launch button with its pin button BESIDE it (never nested); the pin is
 * revealed on hover and whenever the tile has keyboard focus.
 */
...
/* Unpinned pins are revealed (opacity, via [data-agni-reveal]) on hover AND
   keyboard focus — display:none would take them out of the Tab order. */
const PIN_CAN = "inline-flex bg-surface-sunken text-fg-secondary cursor-pointer hover:bg-surface-brand-soft hover:text-fg-brand";
const PIN_FULL = "inline-flex bg-surface-sunken text-[var(--text-quaternary)] cursor-not-allowed";
const TILE_ICON =
...
function AppTile({ app, isPinned, canPin, onPinToggle, onLaunch }: {
  app: LauncherApp; isPinned: boolean; canPin: boolean; onPinToggle: () => void;
  onLaunch?: (e: React.MouseEvent<HTMLButtonElement>) => void;
}) {
  const pinTip = useTip(300);
  const pinLabel = isPinned ? "Unpin from rail" : canPin ? "Pin to rail" : "Rail full — unpin an app first";
  const name = app.label.replace(/\n/g, " ");
  return (
    <div className="group relative grid">
      <button type="button" className={TILE} onClick={onLaunch}>
        <span aria-hidden="true" className={[TILE_ICON, isPinned ? TILE_ICON_PINNED : TILE_ICON_REST].join(" ")}>
      ...
      <button
        ...
        aria-disabled={!isPinned && !canPin ? true : undefined}
        data-agni-reveal={isPinned ? undefined : ""}
      ...
      >
        <i aria-hidden="true" className={"ph " + (isPinned ? "ph-push-pin-slash" : "ph-push-pin")} />
        {pinTip.open && <TipBubble label={pinLabel} side="bottom" decorative />}
      </button>
    </div>
  );
}
...
export const AppSwitcher = forwardRef<HTMLDivElement, AppSwitcherProps>(function AppSwitcher({
  ...
  const base = useStableId(id, "agni-apps");
  useFocusTrap(panel, open, { initialFocus: search });
  useScrollLock(open);

  useEffect(() => { if (open) setQ(""); }, [open]);
  const launch = (app: LauncherApp) => (e: React.MouseEvent<HTMLButtonElement>) => {
    onLaunch?.(app, e);
  ...
  const isPinned = (app: LauncherApp) => pinnedApps.some((p) => p.icon === app.icon && p.label === app.label);
  const pinToggle = (app: LauncherApp) => {
  ...
  return (
    <div onClick={(e) => { if (e.target === e.currentTarget) onClose?.(); }} className={SCRIM} style={{ animation: "agni-fade-in var(--dur-normal) var(--ease-standard)" }}>
      <div ref={mergeRefs(ref, panel)} id={base} role="dialog" aria-modal="true" aria-labelledby={base + "-title"} aria-describedby={base + "-desc"}
        onKeyDown={(e) => { if (e.key === "Escape") { e.stopPropagation(); onClose?.(); } }}
        className={PANEL}
        style={{ animation: "agni-scale-pop var(--dur-normal) var(--ease-spring)" }}>
        {/* Header */}
        <div className={HEADER}>
          <span aria-hidden="true" className={LAUNCH_TILE}><i className="ph-bold ph-dots-nine" /></span>
          <div className="flex-1 min-w-0">
            <h2 id={base + "-title"} className="m-0 font-sans text-md font-semibold text-fg-primary" style={{ letterSpacing: "normal", lineHeight: "inherit" }}>{label}</h2>
            <div id={base + "-desc"} className="text-xs text-fg-tertiary">Jump to any Agnikul desk</div>
          </div>
          <Input
            ref={search}
            type="search"
          ...
          />
          <button type="button" onClick={onClose} aria-label="Close" className={CLOSE}><i aria-hidden="true" className="ph ph-x" /></button>
        </div>
        ...
        <div className="p-6 max-h-[calc(100vh-220px)] overflow-y-auto">
            ...
            <div className="mb-6">
              ...
              <div className="flex flex-wrap gap-2">
                {quickActions.map((a, i) => (
                  <button key={i} type="button" className={QUICK} onClick={launch(a)}>
                    <i aria-hidden="true" className={"ph " + a.icon + " text-[17px]"} /> {a.label}
                  </button>
                ))}
              </div>
            </div>
          ...
          {pinnedApps.length > 0 && !ql && (
            <div className="mb-6">
              <div className={SECTION_HEAD}>
                <i aria-hidden="true" className="ph ph-push-pin-fill text-[16px] text-fg-brand" />
              ...
              <div className={GRID}>
                {pinnedApps.map((app, i) => (
                  <AppTile key={"pin" + i} app={app} isPinned={true} canPin={false} onLaunch={launch(app)}
          ...
          {filtered.map((cat) => (
            <div key={cat.key} className="mb-6">
              <div className={SECTION_HEAD}>
                <i aria-hidden="true" className={"ph " + cat.icon + " text-[18px] text-fg-brand"} />
                <span className="text-sm font-semibold text-fg-primary">{cat.label}</span>
                <span className={SECTION_RULE} />
              </div>
              <div className={GRID}>
                {cat.apps.map((app, i) => (
                  <AppTile key={cat.key + i} app={app} onLaunch={launch(app)}
          ...
          {filtered.length === 0 && (
            <div role="status" className="text-center py-12 text-fg-tertiary">
              <i aria-hidden="true" className="ph ph-magnifying-glass text-[32px] opacity-[0.4] block mb-2" />
