...
export function CopyButton({ value, title = "Copy" }) {
  ...
  return (
    <button type="button" title={copied ? "Copied" : title} onClick={copy}
      ...
      <i className={copied ? "ph-fill ph-check-circle" : "ph ph-copy"} />
    </button>
  );
}

export function KeyValueRow({ icon, label, value, copyable, mono, onClick, span, layout = "stacked", last, style }) {
  const val = onClick
  ...
  return (
    <div style={{ gridColumn: span ? "1 / -1" : undefined, padding: layout === "bordered" ? "9px 12px" : 0, borderBottom: layout === "bordered" && !last ? "1px solid var(--border-subtle)" : undefined, ...style }}>
...
function DetailListBody({ items = [], layout = "bordered", minCol = 200, style }) {
  ...
  return (
    <div style={{ background:"var(--surface-card)", border:"1px solid var(--border-subtle)", borderRadius:"var(--radius-md)", overflow:"hidden", ...style }}>
