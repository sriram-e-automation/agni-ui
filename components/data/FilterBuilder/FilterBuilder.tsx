...
export const FilterBuilder = React.forwardRef<HTMLDivElement, FilterBuilderProps>(function FilterBuilder({ fields = [], value, onChange, style = {} }, ref) {
  ...
  return (
    <div ref={ref as never} className="bg-surface-card border border-line-subtle rounded-lg p-3" style={style}>
      <div className="flex flex-col gap-2">
        {rules.map((r, i) => (
          <div key={i} role="group" aria-label={"Rule " + (i + 1)} className="flex items-center gap-2">
            <span className="text-xs text-fg-tertiary w-[36px] shrink-0">{i === 0 ? "Where" : "and"}</span>
            <select aria-label="Field" value={r.field} onChange={(e) => update(i, { field: e.target.value })} className={[SEL, "flex-1 min-w-0"].join(" ")}>
              {fields.map((f) => <option key={f.key} value={f.key}>{f.label}</option>)}
            </select>
            <select aria-label="Operator" value={r.op} onChange={(e) => update(i, { op: e.target.value })} className={[SEL, "w-[110px]"].join(" ")}>
              {OPS.map((o) => <option key={o} value={o}>{o}</option>)}
            </select>
            <input aria-label="Value" value={r.val} onChange={(e) => update(i, { val: e.target.value })} placeholder="value"
              className={[SEL, "flex-1 min-w-0"].join(" ")} />
            <button type="button" aria-label={"Remove rule " + (i + 1)} onClick={() => remove(i)} disabled={rules.length === 1} className={REMOVE}>
