...
| `components/<family>/<Name>/` | **One folder per component** — `<Name>.tsx` (implementation) · `<Name>.d.ts` (published contract) · `index.ts` (the public entry: the component + its contract types) · any `@internal` renderers that exist only to serve it (e.g. `primitives/Button/` also holds `ButtonBase` · `IconButton` · `SplitButton`). Import from the folder (`components/primitives/Button`); inside the library, files import each other's `.tsx` directly. Specimen cards (`*.card.html`) stay one level up, in the family folder. |
| `components/primitives/` | The atoms every other family composes: `Button` · `Badge` (internal) · `Tag` · `Avatar` · `AvatarStack` · `Rating` (star scale, read-only or settable) · `Input` · `Textarea` · `Checkbox` · `Radio`/`RadioGroup` · `Switch` · `Select` |
| `components/utils/` | **Shared logic and contracts with no visual of their own** — `actionSpec` (the `ActionSpec` / `ExportActionSpec` contract every grouped component takes its actions as · `renderActions` · `visibleActions` · `exportToAction`) · `DataState` / `resolveDataState` (the error → loading → empty → content precedence) · `ChartKit` (the chart engine) · `Theme` (mode · accent · density helper) · `RoleGate` / `roleAllows` (role-based visibility) · `panelState` (`resolvePanelBody`) · **`form`** (`fieldProps` — the React Hook Form / Formik adapter) · **`field`** (`FieldContext` / `useFieldControl` — how FormField wires label, hint, error and invalid state into its control) · **`interaction`** (the shared hooks every interactive component is built on: `useStableId` · `mergeRefs` · `composeHandlers` · `useControllableState` · `useFocusTrap` · `useListNavigation` · `useRovingFocus` · `pressableProps`). One barrel: `components/utils/index.ts`. |
| `components/forms/` | `DatePicker` · `QuantityStepper` · `RichTextEditor` · `FileUpload` · `FormField` (label + control + hint/error) · `FormSection` (brand-rule heading + field grid). The single-element controls live in `primitives/`. |
| `components/navigation/` | `Tabs` · `Breadcrumbs` · `CommandPalette` · `DropdownMenu` · `ActionTile` (module landing-page launchpad tile) |
| `components/data/` | `DataTable` · `List` · `TreeView` · `Pagination` · `EditableTable` · `GanttTimeline` (grouped scheduling bars — group-by rows · navigator row with ‹ Today › paging + day/week/month/quarter/year toggle · status-toned bars · today line) · `Calendar` (record calendar view type — month/week/year grid keyed by each record's date · status-toned per-day count chips · searchable selected-day panel · ‹ Today › nav) · `StageList` (ordered stage pipeline — five states · row actions) · `QuickStats` (click-to-filter stat card strip) · `DateRangeFilter` (granularity tabs + period navigator + custom range popover) · `FilterPanel` (facet popover — the facets half of the filter pair) · `FilterBuilder` (AND-chained rule builder — the rules half) · `PageControls` (the one records toolbar — search · facet filters · date range · sort · column chooser · view switcher · declarative export/primary/secondary actions · bulk-selection bar · pagination) · **`RecordTable`** (the whole records region — PageControls + DataTable + per-row action column + selection + pagination) · **`StatsOverview`** (the dashboard block — QuickStats strip + auto-fit ChartCard grid) · `RecordCard` (one record contract, presets kanban · approval · task) · `KanbanBoard` / `PersonCard` · `OrgTree` · `BulkActionToolbar` · `DocumentPreview` · **the record-detail family:** `DetailSection` (read-only counterpart to FormSection) · `DetailList` + `KeyValueRow` + `CopyButton` · `StatCard` (one measured figure) · `AttachmentRow` (file row with uploading/failed states) · `EventRow` (agenda entry on the shared tone vocabulary) · `OptionRow` (icon + title + description choice row — trackers, support options, settings) · `StatusChip` (internal) |
| `components/charts/` | `BarChart` · `LineChart` · `PieChart` · `DonutChart` · `ScatterChart` · `SparkLineChart` · `Gauge` · `RadarChart` · `Heatmap` (engine: `utils/ChartKit`) · `ChartCard` (title · subtitle · legend framing) — **MUI X Charts–compatible API** (`series` / `xAxis` / `yAxis` / `radar`), themed SVG with animated entrance (bars rise · arcs sweep · lines draw · slices & cells pop) + highlight-on-hover, interactive legends, light+dark via the `--chart-*` palette, all reduced-motion aware. No CSS-in-JS / external lib. |
| `components/feedback/` | `Notice` (inline banner · toast) · `Tooltip` · `Modal` · `Progress` · `EmptyState` · `ErrorState` (failure block — message · detail · retry; consumed by DataTable / KanbanBoard / Card / RecordDetailModal `error` props) · **`Loading`** (shape-matched skeleton for every component family, + `Spinner` / `LoadingOverlay`). The state resolver they all share is `utils/DataState`. |
| `components/containment/` | `Panel` (inline · `variant="sheet"` bottom slide-up · `variant="drawer"` side panel over a scrim) · `Accordion` · `Card` |
...
| `components/chrome/` | `ShellHeader` · `ShellFooter` · `NavRail` (drawer ⇄ icon rail ⇄ overlay — overlay mode owns its own scrim and slide-in panel) · `PageTitleBar` · **`PageHeader`** · `WorkspacePane` · `AppSwitcher` · `SettingsMenu` · `NotificationsMenu` · `PanelKit` |
...
**How the families are organised.** Families are named after the **shape** of what they hold, not the domain they serve — `primitives/` the single-element atoms everything else composes, `data/` anything that displays records, `chrome/` the persistent frame. Logic with no visual of its own is not a component and lives in `utils/`. The one exception is **`workflow/`**, the approval layer that genuinely knows what a request, approval and audit entry are; if a component would work unchanged in a product with no approvals, it does not belong there. (Aug 2026: `erp/` was retired under this rule — 9 domain-bound pieces to `workflow/`, 9 shape-generic ones to `data/`, `RoleGate` to `core/`. Folder moves don't change the runtime namespace; every component is still reached flat off the bundle.) (Sep 2026: `core/` was retired — its atoms and the single-element form controls (`Input` · `Textarea` · `Checkbox` · `Radio` · `Switch` · `Select`) moved to `primitives/`; `Card` to `containment/`, `DropdownMenu` + `ActionTile` to `navigation/`, `OptionRow` to `data/`; `actionSpec` · `DataState` · `ChartKit` · `Theme` · `RoleGate` · `panelState` to `utils/`. Every component now sits in its own folder with an `index.ts`.)
...
- **primitives** — Button · Tag · Avatar · **AvatarStack** · **Rating** · Input · Textarea · Checkbox · Radio · RadioGroup · Switch · Select
- **forms** — DatePicker · QuantityStepper · RichTextEditor · FileUpload · **FormField** · **FormSection**
- **navigation** — Tabs · Breadcrumbs · CommandPalette · DropdownMenu · **ActionTile**
...
- **charts** — BarChart · LineChart · PieChart · DonutChart · ScatterChart · SparkLineChart · Gauge · RadarChart · Heatmap · **ChartCard** (title · subtitle · legend framing for one chart)
- **feedback** — **Notice** (inline banner · toast) · Tooltip · Modal · Progress · EmptyState · **ErrorState** · **Loading**
- **containment** — Panel (inline · sheet · drawer) · Accordion · Card
...
- **chrome** — **ShellHeader** · **ShellFooter** · **NavRail** · **PageTitleBar** · WorkspacePane · AppSwitcher · SettingsMenu · NotificationsMenu · **PanelKit** (PanelIconMenu · MenuRow · PanelEmpty)
- **utils** (not components) — **actionSpec** · **DataState** (the state contract in one place — every data component resolves error → loading → empty → content through it) · ChartKit · Theme · RoleGate · panelState
...
**Every group takes its actions as data, not JSX.** `primaryAction`, `secondaryActions` and `exportAction` are `ActionSpec` / `ExportActionSpec` objects (`components/utils/actionSpec`), so the group role-gates, sizes and disables its own actions instead of trusting each page to do it the same way. The `actions` ReactNode slot survives on every group as the escape hatch and renders after the declarative set.
...
### The interaction contract (Sep 2026)

Every interactive component meets the same contract, implemented once in `components/utils/interaction.tsx` rather than per component.

- **Ids.** Generated with `useId` (SSR-safe); a caller's `id` always wins. Inside a `FormField` the control takes the field's id, so `<label for>` works, and gets `aria-describedby` → the hint or the error, `aria-invalid`, `required` and `disabled` without the page passing an id.
- **Refs.** Every component forwards its ref to the element you would expect to focus or measure: the native `<input>` (Input, Textarea, Checkbox, Radio, Switch, QuantityStepper), the `<button>` (Button, DatePicker trigger, ActionTile), the focusable combobox (Select), the dialog panel (Modal, Panel sheet/drawer, CommandPalette, RecordDetailModal, ConfirmModal), and the root element everywhere else.
- **Events and native attributes.** Components extend the matching `React.*HTMLAttributes`, so `onFocus`, `onBlur`, `onKeyDown`, `name`, `form`, `autoComplete`, `aria-*` and the rest reach the DOM. Consumer handlers are **composed** with the component's own: the consumer runs first, and `e.preventDefault()` skips the built-in behaviour (e.g. keeps a menu open). Composite controls fire `onBlur` only when focus leaves the whole control.
- **Controlled or uncontrolled.** `value` / `checked` / `open` control; `defaultValue` / `defaultChecked` / `defaultOpen` don't. That is what lets React Hook Form's `register()` drive a component.
- **Focus.** Dialogs trap Tab, move focus in on open, return it to the opener on close, lock page scroll, and close on Escape from the dialog itself — so nested dialogs close one at a time. Popovers (filters, date range, settings, notifications) move focus in and back the same way. Hover-revealed affordances also appear on keyboard focus (`[data-agni-reveal]`).
- **Pattern by purpose** (WAI-ARIA APG):

| Component | Pattern | Keys |
| --- | --- | --- |
| `Select` (plain · multiple) | select-only **combobox + listbox** (`aria-activedescendant`) | ↓ ↑ Enter Space open · arrows · Home/End · PageUp/Down · typeahead · Enter select · Esc · Backspace clears / drops last chip |
| `Select` (searchable · creatable · users) | combobox trigger + **filtering listbox** driven from the search box | typing seeds the search · arrows · Enter · Esc back to trigger |
| `DropdownMenu`, `Button items`, ShellHeader profile | **menu button + menu** | ↓/Enter/Space open on first, ↑ on last · arrows · Home/End · typeahead · Esc returns focus · Tab closes |
| `CommandPalette` | modal dialog around a **combobox + listbox** | arrows · PageUp/Down · Enter run · Esc |
| `Tabs` (page strip) | **tablist** (automatic or `activation="manual"`), `Tabs.panelProps()` for panels | ← → · Home/End |
| `Tabs` pill track, PageTitleBar scope/view, `Rating` | **radio group** with roving focus | arrows move and select |
| `Checkbox` · `Radio` · `Switch` | real native inputs (`role="switch"` on Switch) | Space · arrows within a named radio group |
| `QuantityStepper` | **spinbutton** | ↑ ↓ · PageUp/Down · Home/End |
| `DatePicker` | **date picker dialog** with a day **grid** | ← → day · ↑ ↓ week · Home/End · PageUp/Down month (Shift: year) |
| `Accordion`, collapsible `Panel`, NavRail groups | **disclosure** (`aria-expanded` / `aria-controls`) | Enter/Space · ↑ ↓ between accordion triggers |
| `TreeView` | **tree** | ↑ ↓ · → expand/child · ← collapse/parent · Home/End · typeahead |
| `Modal` · `Panel` sheet/drawer · `AppSwitcher` · NavRail overlay | **modal dialog** | Tab trapped · Esc |
| `RichTextEditor`, WorkspacePane | **toolbar** (roving) + multi-line textbox | Alt+F10 to toolbar · Esc back · Ctrl/⌘+B/I/U/K |
| `DataTable` | table with sort **buttons** + `aria-sort`, labelled row checkboxes, focusable clickable rows | Enter opens a row |

Cards, calendar cells, Gantt rows and other click targets that can't be a `<button>` use `pressableProps` (role, tabindex, Enter/Space — only when the key lands on the element itself).
...
5. **Forms.** Controls call `onChange(value, event?)` — value first. With React Hook Form's `register()` or Formik's `field`, spread through the adapter: `<Input {...fieldProps(register("email"))} />`, `<Checkbox {...fieldProps(formik.getFieldProps({ name: "agree", type: "checkbox" }))} />`. RHF's `<Controller>` needs no adapter (`render={({ field }) => <Select {...field} options={…} />}`), and its `field.ref` gets focus-on-error. Every control also submits with a plain `<form>` when given a `name` (hidden inputs for the custom ones).
6. Toggle dark mode with `data-theme="dark"` and density with `data-density="compact|comfortable|spacious"` on a container; apply an accent with `style={{ ...Theme.deriveAccentVars(accent, dark) }}`. Set display size with `data-scale="sm|md|lg|xl"` (on `<html>` for the whole desk, or any subtree).
